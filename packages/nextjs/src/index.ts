...
export default (ctx: IPluginContext, pluginOpts: PluginOptions) => {
    ...
    const outputPath = copyFile ? appPath : paths.outputPath;
    ...
    ctx.registerCommand({
        ...
        synopsisList: ["taro start -p <port>"],
        fn() {
    ...
    ctx.registerPlatform({
        ...
        async fn({ config }) {
            ...
            const appConfigFilePath = resolveScriptPath(
                path.join(sourcePath, `${helper.ENTRY}.config`)
            );
            const appConfig = helper.readConfig(appConfigFilePath);
            const appFilePath = resolveScriptPath(
                path.join(sourcePath, helper.ENTRY)
            );

            const outputSourcePath = path.join(outputPath, "src");
            const outputAppFilePath =
                path.join(outputSourcePath, helper.ENTRY) +
                path.extname(appFilePath);

            const nextAppFilePath = path.join(outputPath, "pages/_app.jsx");

            const templateDir = path.resolve(__dirname, "../template");

            const taroPages = mergeTaroPages(
                appConfig.pages,
            ...
            if (router.customRoutes) {
                for (const key of Object.keys(router.customRoutes)) {
                    customRoutes[ensureLeadingSlash(key)] =
                        router.customRoutes[key];
                }
            }

            const additionalData = await getNextSassOptions(sass);

            const nextPageInfos = await getNextPageInfos(
                sourcePath,
                taroPages,
                customRoutes
            );
            ...
            function createNextjsPages() {
                ...
                for (const pageInfo of nextPageInfos) {
                    ...
                    const nextjsPageFilePath = path.join(
                        nextjsPagesDir,
                        targetPageFile
                    );
                    ...
                    const exportedFunctions = getNextExportedFunctions(
                        path.join(sourcePath, pageInfo.file)
                    );
                    ...
                    const configAbsolutePath = helper.resolveMainFilePath(
                        `${request}.config`
                    );
                    ...
                    fs.writeFileSync(nextjsPageFilePath, contents, {
                        encoding: "utf-8",
                    });
                }

                // 处理自定义路由
                const customRoutesFilePath = path.join(
                    outputPath,
                    "customRoutes.json"
                );
                fs.writeFileSync(
                    customRoutesFilePath,
                    JSON.stringify(customRoutes, null, "  "),
                    { encoding: "utf-8" }
                );

                // 处理 next.js 错误处理页面
                for (const errorPage of ["_error", "404", "500"]) {
                    const pagePath = resolveScriptPath(
                        path.join(sourcePath, errorPage)
                    );
                    if (fs.existsSync(pagePath)) {
                        const modulePath = path.normalize(
                            path.relative(
                                nextjsPagesDir,
                                path.join(outputSourcePath, errorPage)
                            )
                        );
                        const contents = `export {default} from '${modulePath}'`;
                        const nextjsErrorPagePath = path.join(
                            nextjsPagesDir,
                            `${errorPage}.js`
                        );
                        fs.writeFileSync(nextjsErrorPagePath, contents, {
                            encoding: "utf-8",
                        });
                    }
                }
            ...
            function scaffold() {
                ...
                return es.merge(
                    ...
                    src(`${templateDir}/pages/_document.jsx`).pipe(
                        dest(path.join(outputPath, "pages"))
                    ),
                    src(`${appPath}/config/**`).pipe(
                        dest(path.join(outputPath, "config"))
                    ),
                    src(`${templateDir}/next.config.ejs`)
                        .pipe(
                            es.through(function (data) {
                                ...
                                const rewrites =
                                    resolveDynamicPagesToRewrites(dynamicPages);

                                const customNextConfigPath = path.join(
                                    path.dirname(configPath),
                                    "next.config.js"
                                );
                                let customNextConfig: string | null = null;
                                if (fs.existsSync(customNextConfigPath)) {
                                    customNextConfig = path.relative(
                                        outputPath,
                                        customNextConfigPath
                                    );
                                }

                                const multipleRoutePages = Object.values(
                                    customRoutes
                                ).filter((value) =>
                                    Array.isArray(value)
                                ) as string[][];
                                for (const multipleRoute of multipleRoutePages) {
                                    for (const source of multipleRoute.slice(
                                        1
                                    )) {
                                        rewrites.push({
                                            source: ensureLeadingSlash(source),
                                            destination: ensureLeadingSlash(
                                                multipleRoute[0]
                                            ),
                                        });
                                    }
                    ...
                    src(`${templateDir}/postcss.config.ejs`)
                        .pipe(
                            es.through(function (data) {
                                const plugins = Object.entries(postcss).reduce(
                                    (result, info) => {
                                        const [pluginName, pluginOption] =
                                            info as any;
                                        if (
                                            !DEFAULT_POSTCSS_OPTIONS.includes(
                                                pluginName
                                            ) &&
                                            pluginOption?.enable
                                        ) {
                                ...
                                const autoprefixerOption = merge(
                                    {},
                                    DEFAULT_AUTOPREFIXER_OPTION,
                                    postcss.autoprefixer
                                );
                                const ejsData = {
                                    designWidth,
                                    autoprefixerOption:
                                        autoprefixerOption.enable
                                            ? JSON.stringify(
                                                  autoprefixerOption.config
                                              )
                                            : null,
                                    plugins,
                                };
                    ...
                    src(`${templateDir}/babel.config.ejs`)
                        .pipe(
                            es.through(function (data) {
                                const ejsData = {
                                    nextAppFilePath: JSON.stringify(
                                        path.relative(
                                            outputPath,
                                            nextAppFilePath
                                        )
                                    ),
                                    outputAppFilePath: JSON.stringify(
                                        path.relative(
                                            outputPath,
                                            outputAppFilePath
                                        )
                    ...
                    src(`${templateDir}/tsconfig.json`)
                        .pipe(
                            es.through(function (data) {
                                const taroTSConfigPath = path.join(
                                    appPath,
                                    "tsconfig.json"
                                );
                                if (fs.existsSync(taroTSConfigPath)) {
                                    const taroTSConfig =
                                        parseJson(taroTSConfigPath);
                                    const templateTSConfig = parseJson(
                                        data.path
                                    );

                                    let mergedTSConfig = templateTSConfig;
                                    const paths = resolveAliasToTSConfigPaths(
                                        alias,
                                        taroTSConfigPath
                                    );
                                    mergedTSConfig = merge(
                                        taroTSConfig,
                                        templateTSConfig,
            ...
            scaffold().on("end", async () => {
                console.log(
                    chalk.green(
                        "Compiled to Next.js application successfully!\n"
                    )
                );
                ...
                if (devScripts) {
                    if (mode === "development") {
                        spawn("npm", ["run", devScripts, "-p", port], {
                            cwd: outputPath,
                            stdio: "inherit",
                        });
                    }
                ...
                if (buildScripts) {
                    if (mode !== "development") {
                        spawn("npm", ["run", buildScripts, "-p", port], {
                            cwd: outputPath,
                            stdio: "inherit",
                        });
                    }
                ...
                if (isWatch && devServer.open) {
                    openBrowser(devServer.open);
                }
                if (isWatch && !copyFile) {
                    function hasSpecifiedFile(filePath: string): boolean {
                        ...
                        return files.some((name) => {
                            ...
                            const secondaryBase = path.basename(
                                primaryBase,
                                secondaryExt
                            );
                    ...
                    function handleWatch(
                        operation: "changed" | "added" | "removed",
                        filePath: string
                    ): void {
                    ...
                    const watcher = watch(`${path.normalize(sourcePath)}/**`, {
                        delay: 200,
                    });
                    watcher.on("ready", () =>
                        console.log(
                    ...
                    watcher.on("change", (filePath) =>
                        handleWatch("changed", filePath)
                    );
                    watcher.on("add", (filePath) =>
                        handleWatch("added", filePath)
                    );
                    watcher.on("unlink", (filePath) =>
                        handleWatch("removed", filePath)
                    );
