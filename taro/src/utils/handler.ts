type TCallback<T = any> = (res: T) => Promise<void> | void
interface IMethodParam<T = any> {
    name: string
    success?: TCallback<T>
    fail?: TCallback
    complete?: TCallback
...
export class MethodHandler<T = any> {
    methodName: string
    ...
    success<U = Record<string, unknown>>(res: Partial<T> & any = {}, resolve = Promise.resolve.bind(Promise)): Promise<T & U & any> {
        if (!res.errMsg) {
            res.errMsg = `${this.methodName}:ok`
        }
        typeof this.__success === 'function' && this.__success(res as T)
        typeof this.__complete === 'function' && this.__complete(res)
        return resolve(res as Required<T & U & any>)
    }

    fail<U = Record<string, unknown>>(res: Partial<T> & any = {}, reject = Promise.reject.bind(Promise)): Promise<T & U & any> {
        if (!res.errMsg) {
